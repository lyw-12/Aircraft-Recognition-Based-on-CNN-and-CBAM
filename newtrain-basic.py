...
# 参数设置
parser = argparse.ArgumentParser(description='飞行器图像分类基础CNN（简化版）')
parser.add_argument('--batch_size', default=32, type=int, help='批处理大小')
parser.add_argument('--epochs', default=15, type=int, help='训练轮数')
...
parser.add_argument('--save_dir', default='newmodels-basic', type=str, help='模型保存路径')
...
parser.add_argument('--k_folds', default=5, type=int, help='K折交叉验证中的K值')
parser.add_argument('--use_cv', action='store_true', help='是否使用K折交叉验证')
parser.add_argument('--patience', default=5, type=int, help='早停耐心值，连续多少个epoch验证集性能不提升就停止训练')
...
class BasicAircraftCNN(nn.Module):
    def __init__(self, num_classes=4):
        ...
        # 全连接分类器 - 减少神经元数量
        # 从140x140经过两次池化后变为35x35
        self.fc1 = nn.Linear(32 * 35 * 35, 128)  # 注意修改了输入维度
        self.relu3 = nn.ReLU()
        self.dropout = nn.Dropout(0.25)  # 减小dropout率，允许更多过拟合
